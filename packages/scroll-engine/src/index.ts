...
/**
 * Per-orientation mappings. Portrait needs its own pacing: thumb gestures cover less
 * distance per effort, so holds that read as a beat on desktop read as a stall on a phone.
 */
export type BeatsOption = Beat[] | { landscape: Beat[]; portrait: Beat[] };
...
export class ScrollEngine {
  ...
  /** The beat list for the orientation currently on screen. */
  private beatsForOrientation(): Beat[] | undefined {
    const b = this.o.beats;
    if (!b) return undefined;
    if (Array.isArray(b)) return b;
    return this.usingPortrait ? b.portrait : b.landscape;
  }
  ...
  private frameFor(progress: number): number {
    ...
    const beats = this.beatsForOrientation();
